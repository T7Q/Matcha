...
const getUserConversations = async (userId) => {
    const result = await db.query(
        ...
        LEFT JOIN messages m1 ON chats.chat_id = m1.chat_id AND m1.message_id = (
            SELECT MAX(m2.message_id) FROM messages m2 WHERE m2.chat_id = chats.chat_id)
...
const isChatExists = async (chatId) => {
    return await db.query(`SELECT count(chat_id) FROM chats WHERE chat_id = $1`, [chatId]);
};
...
const addMessage = async (chatId, senderId, content, receiverId) => {
    await db.query(
        `INSERT INTO notifications (to_user_id, from_user_id, type) VALUES($1, $2, $3);`,
