...
const getCompatibilityValue = (signUser1, signUser2, table) => {
    ...
    let query = `(SELECT compatibility_value::float * 10 as Cn
        FROM ${table} WHERE (sign_1 = ${signUser1} AND sign_2 = '${signUser2}')
        OR (sign_1 = '${signUser2}' AND sign_2 = ${signUser1}))`;
    return query;
};

const getConnectionValue = (userId1, userId2) => {
    userId1 = 'users.user_id';
    let query = `(CASE WHEN ((SELECT count(likes.like_id) AS from_likes FROM likes WHERE likes.from_user_id = ${userId1}
        AND likes.to_user_id = ${userId2}) = 1 AND (SELECT count(likes.like_id) AS to_likes FROM likes
        WHERE likes.from_user_id = ${userId2} AND likes.to_user_id = ${userId1}) = 1) THEN 2
        WHEN ((SELECT count(likes.like_id) AS to_likes FROM likes WHERE likes.from_user_id = ${userId1}
        AND likes.to_user_id = ${userId2}) = 1) THEN 3 WHEN ((SELECT count(likes.like_id) AS to_likes
        FROM likes WHERE likes.from_user_id = ${userId2} AND likes.to_user_id = ${userId1}) = 1) THEN 1 ELSE 0 END) as connected`;
    return query;
};
